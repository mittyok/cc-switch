...
async fn handle_responses_for_app(
    ...
    if matches!(app_type, AppType::Codex)
        && pipeline_mode == crate::settings::CodexClaudePipelineMode::Always
    {
        log::info!(
        ...
        return handle_responses_via_claude_pipeline(
            state, body, headers, extensions, method, &uri,
        )
        .await;
    }

    let is_fallback = matches!(app_type, AppType::Codex)
        && pipeline_mode == crate::settings::CodexClaudePipelineMode::Fallback;
    ...
    // Try providers first; in fallback mode, provider-down errors redirect to Claude
    let ctx_result =
        RequestContext::new(&state, &body, &headers, app_type.clone(), tag, app_type_str).await;
...
async fn handle_responses_compact_for_app(
    ...
    if matches!(app_type, AppType::Codex)
        && crate::settings::codex_claude_pipeline_mode()
            == crate::settings::CodexClaudePipelineMode::Always
    {
        log::info!(
    ...
    let is_fallback = matches!(app_type, AppType::Codex)
        && crate::settings::codex_claude_pipeline_mode()
            == crate::settings::CodexClaudePipelineMode::Fallback;
...
async fn handle_responses_via_claude_pipeline(
        ...
        .await
    {
        ...
        Err(mut err) => {
            ...
            log::error!("[Codex→Claude] Upstream error: {:?}", &err.error);
            log_forward_error(&state, &ctx, is_stream, &err.error);
    ...
    if api_format == "openai_responses" {
        log::debug!(
            "[Codex→Claude] Provider uses openai_responses, short-circuiting (passthrough)"
        );
...
async fn convert_anthropic_response_to_responses(
    ...
    let responses_body = transform_responses::anthropic_response_to_responses(anthropic_response)
        .map_err(|e| {
...
async fn convert_anthropic_error_to_responses(
    ...
    let body = serde_json::to_vec(&responses_error)
        .map_err(|e| ProxyError::TransformError(format!("Failed to serialize error body: {e}")))?;
...
/// (xAI): restore flattened `function_call` names produced by request-side
/// namespace flattening and rewrite whole-float tool arguments. Error bodies
/// pass through unchanged. Usage is collected exactly as `process_response`
/// would (same `CODEX_PARSER_CONFIG`).
...
pub async fn handle_embeddings(
    ...
    let request_model = request_params
        .get("model")
        .and_then(|v| v.as_str())
        .unwrap_or("unknown");
    let input_texts = request_params
        .get("input")
        .map(|v| {
